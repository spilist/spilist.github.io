...
우리의 실제 앱들은 엄청나게 복잡하지만 저는 아주 단순한 예시를 사용할 겁니다. 이 예시는 컴포넌트가 4개뿐입니다. 한 경로에서 다른 경로로 어떻게 움직일지에 대한 정보가 모여있는 라우터 하나와, A, B, C라는 루트 컴포넌트입니다.

앞에서 말했듯 이 예시 앱은 중앙집중적 import 문제를 가질 수 있습니다. 라우터가 모든 루트 컴포넌트들을 import하기 때문에, 루트 컴포넌트 하나를 지우려면 라우터에 가야 하고, 거기서 import문을 지워야 하고, 경로 정의도 지워야 하고, 결국 "2017 특별 휴일 문제"를 겪게 됩니다.

구글에서는 이 문제의 해결책으로, 지금까지 한번도 공개된 적 없는 컨셉을 만들었습니다. `enhance`라는 건데요. `enhance`는 import 대신 사용함으로써 역 의존성을 나타내기 위한 것입니다. 한 컴포넌트가 모듈을 enhance하면, 그건 그 모듈이 컴포넌트에 대한 의존성이 있음을 나타냅니다.

![img](https://cdn-images-1.medium.com/max/1600/1*bDH4yzG0mrrYlrs2C9twsA.png)
