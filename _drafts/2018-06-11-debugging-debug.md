...
- **언제 전달되지 않을 수 있는가?** 재현 조건을 찾으면서 어떤 컨텐츠가 우측에 있느냐에 영향을 받는다는 사실을 알았다. 유투브와 PDF를 렌더하는 코드에서 대신 빈 컨텐츠를 로드했더니 정상적으로 리사이즈가 된다는 것을 확인했다. 이 두 타입의 공통점은 다른 컨텐츠와 다르게 `iframe` 을 활용한다는 것이었다. 그런데 `iframe`은 `Resizable`을  호출한 `window`와 다른 `window`이므로 이벤트가 전달되지 않는다!
