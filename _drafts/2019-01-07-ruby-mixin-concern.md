...
*(글의 주제에 집중하기 위해 일부러 디테일을 많이 생략했는데, 이 글에서 다루지 않은 부분이 궁금하시다면 **참고** 섹션의 링크들을 읽어보시길 바랍니다.)*
...
MyClass.new.sum_of([1, 2, 3]) # => sum_of([1, 2, 3]) finished: 6
```
...
## 레일즈의 mixin - `ActiveSupport::Concern` ##
...
```
 ...
 `Record` 클래스가 `DisabledModule`을 include함으로써 다음 세 가지가 가능해진다.

- `Record.new.disabled?`: 모듈이 include됐으므로 인스턴스 메서드가 확장된다.
- `Record.available_list`: Record 클래스가 `DisabledModule::ClassMethods`를 extend하기 때문에 클래스 메서드가 확장된다.
...
루비 1.9.3에서 included hook이 도입되고부터 이런 식으로 scope, association, class method 등을 재사용하는 패턴이 많아졌다. 이 패턴을 간편하게 줄여서 쓸 수 있게 해주는 것이 레일즈 4부터 생긴 `ActiveSupport::Concern`이다. `ActiveSupport::Concern`을 extend한 모듈은 `included` 블록으로 `class_eval`을 생략할 수 있으며, `base.extend ClassMethods` 대신 `class_methods` 블록 안에 메서드를 정의할 수 있게 된다. 그래서 위 
...
여기까지만 해도 훌륭하지만, `ActiveSupport::Concern`은 모듈간 의존성 문제도 잘 해결해준다. 다음 예시를 보자.
...
위 코드를 실행하면 `NoMethodError (undefined method 'has_many' for MyModuleB:Module)` 라고 에러가 뜬다. 이는 
